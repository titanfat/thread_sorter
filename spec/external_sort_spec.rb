...
RSpec.describe ThreadSorter::ExternalSort do

  let!(:generated_data) { ThreadSorter::DataGenerator.generate_csv(100) }
  ...
  subject { ThreadSorter::ExternalSort.new(generated_data, 'test_transaction.csv') }

  it 'serialized data transaction' do
    raw_transaction = File.foreach(generated_data).first
    ins = subject.parse_line(raw_transaction)

    expect(ins.class).to eq(ThreadSorter::Entity::TransactionCsv)
  ...
  it 'sorting by temporary file divided into parts' do
    init_sort = ThreadSorter::ExternalSort.new(generated_data, 'test_transaction.csv')
    init_sort.sort
